  ...
  window.decoMistape = $.extend(window.decoMistape, {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          if (content.hasClass("with-comment")) {
            dialogWrap.css(
              "transform",
              "translateY(12%) scaleY(0.5) scaleX(0.4)"
            );
          } else {
            dialogWrap.css(
              "transform",
              "translateY(28%) scaleY(0.5) scaleX(0.45)"
            );
          }
          setTimeout(function () {
    ...
    getSelectionData: function () {
      ...
      var getExactSelPos = function (selection, context) {
        ...
        if (!backwards) {
          ...
          if (
            context.substring(
              sel.anchorOffset,
              sel.anchorOffset + selection.length
            ) == selection
          ) {
          ...
          if (
            context.substring(
              parentElOffset,
              parentElOffset + selection.length
            ) == selection
          ) {
        ...
        if (
          ...
          context.substring(
            sel.focusOffset,
            sel.focusOffset + selection.length
          ) == selection
        ) {
          return sel.anchorOffset;
        }
      ...
      var getExtendedSelection = function (limit, nodeExtensions) {
        ...
        while (i <= limit) {
          if (
            (selContent = stringifyContent(sel.toString().trim())).length >=
              maxContextLength ||
            isSubstrUnique(selContent, context)
          ) {
            return selContent;
          }
          ...
          // only even iteration
          if (
            (i % 2 == 0 && sel.anchorOffset > 0) ||
            (nodeExtensions.left.length && i < limit / 2)
          ) {
            // reset
            if (backwards) {
              sel.collapseToEnd();
            ...
            (nodeExtensions.right.length && i < limit / 2)
          ) {
        ...
        if (!sel.isCollapsed) {
          ...
          while (
            ...
            if (backwards) {
              ...
              endOffset =
                sel.anchorOffset == 0
                  ? sel.anchorNode.previousSibling.length
                  : sel.anchorOffset;
      ...
      // this one is for IE11
        ...
        while (
          ...
          !startNode.textContent
            .slice(startOffset - 1, startOffset)
            .match(/[\s\n\t]/)
        ) {
          startOffset--;
        }
        while (
          ...
          !endNode.textContent.slice(endOffset, endOffset + 1).match(/[\s\n\t]/)
        ) {
          endOffset++;
        }
      ...
      var selToFindInContext,
        contextsToCheck = {
          // different browsers implement different methods, we try them by turn
      ...
      for (var method in contextsToCheck) {
        if (
          contextsToCheck.hasOwnProperty(method) &&
          typeof contextsToCheck[method] != "undefined"
        ) {
          ...
          if (
            isSubstrUnique(selWord, selWithContext) ||
            selWord == selChars.trim()
          ) {
      ...
      if (scope.context === "extended") {
        context =
          extContext.extensions.left +
          initialContext +
          " " +
          extContext.extensions.right;
      }
      ...
      if (context.length > maxContextLength) {
        if (selPos + selToFindInContext.length / 2 < maxContextLength / 2) {
          ...
          contExcerptEndPos = Math.max(
            selPos + selToFindInContext.length,
            context.indexOf(" ", maxContextLength - 10)
          );
        } else if (
          selPos + selToFindInContext.length / 2 >
          context.length - maxContextLength / 2
        ) {
          selPosInContext = "end";
          contExcerptStartPos = Math.min(
            selPos,
            context.indexOf(" ", context.length - maxContextLength + 10)
          );
          ...
          contExcerptStartPos = Math.min(
            selPos,
            context.indexOf(" ", centerPos - maxContextLength / 2 - 10)
          );
          contExcerptEndPos = Math.max(
            selPos + selToFindInContext.length,
            context.indexOf(" ", centerPos + maxContextLength / 2 - 10)
        ...
        truncatedContext = context
          .substring(contExcerptStartPos, contExcerptEndPos)
          .trim();

        if (
          selPosInContext !== "beginning" &&
          context.charAt(contExcerptStartPos - 1) !== "."
        ) {
          truncatedContext = "... " + truncatedContext;
        }
        if (
          selPosInContext !== "end" &&
          context.charAt(contExcerptStartPos + contExcerptEndPos - 1) !== "."
        ) {
          truncatedContext = truncatedContext + " ...";
        }
      }
      ...
      if (selExactMatch && truncatedContext === context) {
        previewText =
          truncatedContext.substring(0, selPos) +
            selWithContextHighlighted +
            truncatedContext.substring(selPos + selWithContext.length) ||
          selWithContextHighlighted;
      } else {
        previewText =
          truncatedContext.replace(selWithContext, selWithContextHighlighted) ||
          selWithContextHighlighted;
      }
