...
class Mistape_Admin extends Mistape_Abstract
{
	...
	protected function __construct()
	{
		...
		register_uninstall_hook(__FILE__, array('Abstract_Mistape', 'uninstall_cleanup'));
	}
	...
	public function print_options_page()
	{
		...
		$table_name    = $wpdb->base_prefix . Mistape_Abstract::DB_TABLE;
		$reports_count = 0;
...
?>
		<div class="wrap">
			...
			<h2 class="nav-tab-wrapper">
				<?php
				printf(
					'<a href="%s" class="nav-tab%s" data-bodyid="mistape-configuration" >%s</a>',
					add_query_arg('tab', 'configuration'),
					$active_tab == 'configuration' ? ' nav-tab-active' : '',
					__('Configuration', 'mistape')
				);
				printf(
					'<a href="%s" class="nav-tab%s" data-bodyid="mistape-help">%s</a>',
			...
			<?php printf(
				'<div id="mistape-configuration" class="mistape-tab-contents" %s>',
			...
			<form action="<?php echo admin_url('options.php'); ?>" method="post">
				...
				<div id="mistape-sidebar">
					<div id="mistape_statistics" class="postbox -right-sidebar-widget">
						<h3 class="hndle">
		...
		<?php
		printf(
			'<div id="mistape-help" class="mistape-tab-contents" %s>',
	...
	public function register_settings()
	{
		...
		add_settings_field(
			'mistape_email_recipient',
			__('Email recipient', 'mistape'),
			array($this, 'field_email_recipient'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_post_types',
			__('Post types', 'mistape'),
			array($this, 'field_post_types'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_register_shortcode',
			__('Shortcodes', 'mistape'),
			array($this, 'field_register_shortcode'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_caption_format',
			__('Caption format', 'mistape'),
			array($this, 'field_caption_format'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_caption_text_mode',
			__('Caption text mode', 'mistape'),
			array($this, 'field_caption_text_mode'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_show_logo_in_caption',
			__('Icon before the caption text', 'mistape'),
			array($this, 'field_show_logo_in_caption'),
			'mistape_options',
			'mistape_configuration'
		);
		add_settings_field(
			'mistape_color_scheme',
			__('Color scheme', 'mistape'),
		...
		if (is_multisite() && is_main_site()) {
			add_settings_field(
				'mistape_multisite_inheritance',
				__('Multisite inheritance', 'mistape'),
	...
	public function field_email_recipient()
	{
		...
		echo '
			...
			<br>
			<label><input id="mistape_email_recipient-post_author_first" type="checkbox" name="mistape_options[email_recipient][post_author_first]" value="1" ' . checked(
			'yes',
			$this->options['email_recipient']['post_author_first'],
			false
		) . '/>' . __('If post ID is determined, notify post author instead', 'mistape') . '</label>
		</fieldset>';
	}
	...
	public function field_post_types()
	{
		...
		foreach ($this->post_types as $value => $label) {
			echo '
			<label style="padding-right: 8px; min-width: 60px;"><input id="mistape_post_type-' . $value . '" type="checkbox" name="mistape_options[post_types][' . $value . ']" value="1" ' . checked(
				true,
	...
	public function field_register_shortcode()
	{
		echo '
		<fieldset>
			<label><input id="mistape_register_shortcode" type="checkbox" name="mistape_options[register_shortcode]" value="1" ' . checked(
			'yes',
			$this->options['register_shortcode'],
			false
		) . '/>' . __(
			'Register <code>[mistape]</code> shortcode.',
			'mistape'
		) . '</label>
	...
	public function field_caption_format()
	{
		...
		foreach ($this->caption_formats as $value => $label) {
			echo '
			<label><input id="mistape_caption_format-' . $value . '" type="radio" name="mistape_options[caption_format]" value="' . esc_attr($value) . '" ' . checked(
				$value,
	...
	public function field_caption_text_mode()
	{
		...
		foreach ($this->caption_text_modes as $value => $label) {
			echo '<label><input id="mistape_caption_text_mode-' . $value . '" type="radio" name="mistape_options[caption_text_mode]" ' . 'value="' . esc_attr($value) . '" ' . checked(
				$value,
	...
	public function field_multisite_inheritance()
	{
		...
		<fieldset>
			...
			checked(
				...
				false
			) . '/>' . __('Copy settings from main site when new blog is created', 'mistape') . '
	        </label>
		</fieldset>';
	}
	...
	public function validate_options($input)
	{
		...
		if (isset($_POST['option_page']) && $_POST['option_page'] == 'mistape_options') {
			...
			if (
				$input['email_recipient']['type'] == 'admin' && isset($input['email_recipient']['id']['admin']) && (user_can(
				...
				$input['email_recipient']['type'] == 'editor' && isset($input['email_recipient']['id']['editor']) && (user_can(
				...
				if ($invalid_emails) {
					add_settings_error(
						...
						sprintf(
							__('ERROR: You entered invalid email address: %s', 'mistape'),
	...
	public function plugins_page_settings_link($links, $file)
	{
		...
		if ($file == $plugin) {
			array_unshift(
				...
				sprintf(
					'<a href="%s">%s</a>',
					admin_url('options-general.php?page=mistape_settings'),
					__('Settings', 'mistape')
				)
			);
		}
	...
	public function plugin_activated_notice()
	{
		...
		if ($this->options['first_run'] == 'yes' && current_user_can('manage_options')) {
			...
			if ($wp_screen && $wp_screen->id == 'settings_page_mistape') {
				$html .= __(
					'<strong>Mistape</strong> settings notice will be dismissed after saving changes.',
					'mistape'
				);
			} else {
				$html .= sprintf(__(
					'<strong>Mistape</strong> must now be <a href="%s">configured</a> before use.',
					'mistape'
				), admin_url('options-general.php?page=mistape_settings'));
			}
			$html .= '</p>';
	...
	?>
		<div class="card">
			...
			<ul>
				<li>
					<p><code>[mistape format="text" class="mistape_caption_sidebar"]</code></p>
				</li>
			</ul>
			<h4><?php _e('PHP code example:', 'mistape') ?></h4>
			<ul>
				<li>
					<p><code>&lt;?php do_shortcode( '[mistape format="image" class="mistape_caption_footer"
		...
		<div class="card">
			...
			<ul>
				...
				<li class="mistape-hook-block">
					<code>'mistape_mail_recipient', <span class="mistape-var-str">$recipient</span>, <span class="mistape-var-str">$url</span>, <span class="mistape-var-obj">$user</span></code>
				...
				<li class="mistape-hook-block">
					<code>'mistape_custom_email_handling', <span class="mistape-var-bool">$stop</span>, <span class="mistape-var-obj">$ajax_obj</span></code>
					<p class="description"><?php _e(
				...
				<li class="mistape-hook-block">
					...
					<p class="description"><?php _e(
												'Allows to add custom logic for whether to output Mistape to front end or not.',
												'mistape'
											) ?></p>
				</li>
